...
static struct posix_acl *ext4_acl_from_disk(const void *value, size_t size) {
  ...
  for (n = 0; n < count; n++) {
    ext4_acl_entry *entry = (ext4_acl_entry *)value;
...
static void *ext4_acl_to_disk(const struct posix_acl *acl, size_t *size) {
  ...
  *size = ext4_acl_size(acl->a_count);
  ext_acl =
      kmalloc(sizeof(ext4_acl_header) + acl->a_count * sizeof(ext4_acl_entry),
              GFP_NOFS);
  if (!ext_acl)
    return ERR_PTR(-ENOMEM);
...
static int __ext4_set_acl(handle_t *handle, struct inode *inode, int type,
  ...
  error = ext4_xattr_set_handle(handle, inode, name_index, "", value, size,
                                xattr_flags);
...
retry:
  error =
      ext4_xattr_set_credits(inode, acl_size, false /* is_create */, &credits);
  if (error)
    return error;
...
int ext4_init_acl(handle_t *handle, struct inode *inode, struct inode *dir) {
  ...
  if (default_acl) {
    error = __ext4_set_acl(handle, inode, ACL_TYPE_DEFAULT, default_acl,
                           XATTR_CREATE);
